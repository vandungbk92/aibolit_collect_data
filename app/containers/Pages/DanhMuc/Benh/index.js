...
import { add, getById, getAll, delById, updateById } from '@services/danhmuc/benhService';
...
class Benh extends Component {
  ...
  async getDataFilter() {
    ...
    let queryStr = ''
    queryStr += `${search.benh ? '&benh[like]={0}'.format(search.benh) : ''}`
  ...
  handleSaveData = async (data) => {
    let benh = data.benh
    if (!benh.trim()) {
      this.formRef.current.setFieldsValue({ benh: benh.trim() });
    ...
    return <div>
      ...
      <Modal title={_id ? 'Chỉnh sửa' : 'Thêm mới'}
        ...
        <Form ref={this.formRef} id="formModal" name='formModal' autoComplete='off'
          ...
          <Form.Item label="Bệnh" name="benh" hasFeedback labelCol={{ span: 6 }} validateTrigger={['onChange', 'onBlur']}
                     rules={[{ required: true, whitespace: true, message: 'Bệnh không được để trống' }]}>
...
export default withConnect(Benh);

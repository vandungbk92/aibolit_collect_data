...
export default {
  async create(req, res) {
    try {
      ...
      let dataCheck = await SoiDa.findOne({
      ...
      const data = await SoiDa.create(req.body);
  ...
  async findAll(req, res) {
    try {
      ...
      const data = await SoiDa.paginate(query, options)
  ...
  async findOne(req, res) {
    try {
      const { id } = req.params;
      const data = await SoiDa.findOne({is_deleted: false, _id: id})
  ...
  async delete(req, res) {
    try {
      const { id } = req.params;
      const data = await SoiDa.findOneAndUpdate({ _id: id }, {is_deleted: true}, { new: true });
      if (!data) {
          responseAction.error(res, 404, '')
      }
      return res.json(data);
  ...
  async update(req, res) {
    try {
      const { id } = req.params;
      const data = await SoiDa.findOneAndUpdate({ _id: id }, req.body, { new: true })
        .populate({path: 'tinhthanh_id', select: 'tentinh'})
        .populate({path: 'quanhuyen_id', select: 'tenqh'})
        .populate({path: 'phuongxa_id', select: 'tenphuongxa'})
        .populate({path: 'trieuchung_id', select: 'trieuchung'})

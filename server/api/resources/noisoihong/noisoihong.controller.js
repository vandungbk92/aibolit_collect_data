import NoiSoiHong from './noisoihong.model';
...
export default {
  async create(req, res) {
    try {
      ...
      let dataCheck = await NoiSoiHong.findOne({
  ...
  async findAll(req, res) {
    try {
      ...
      const data = await NoiSoiHong.paginate(query, options)
  ...
  async findOne(req, res) {
    try {
      const { id } = req.params;
      const data = await NoiSoiHong.findOne({is_deleted: false, _id: id})
  ...
  async delete(req, res) {
    try {
      const { id } = req.params;
      const data = await NoiSoiHong.findOneAndUpdate({ _id: id }, {is_deleted: true}, { new: true });
      if (!data) {
          responseAction.error(res, 404, '')
      }
      return res.json(data);
  ...
  async update(req, res) {
    try {
      const { id } = req.params;
      const data = await NoiSoiHong.findOneAndUpdate({ _id: id }, req.body, { new: true })
        .populate({path: 'tinhthanh_id', select: 'tentinh'})
        .populate({path: 'quanhuyen_id', select: 'tenqh'})
        .populate({path: 'phuongxa_id', select: 'tenphuongxa'})
        .populate({path: 'trieuchung_id', select: 'trieuchung'})
